...
impl Software {
    pub fn project_seg(&self, v1: &Vec2, v2: &Vec2, camera: &Camera) -> Option<Edge> {
        ...
        // Project to screen X
        let mut sx1 = self.half_w + p1.x * self.focal / p1.y;
        let mut sx2 = self.half_w + p2.x * self.focal / p2.y;
        if (sx1 < 0.0 && sx2 < 0.0) || (sx1 >= self.width_f && sx2 >= self.width_f as f32)
        {
    ...
    pub fn back_facing_seg(&self, p1: &Vec2, p2: &Vec2, camera: &Camera) -> bool {
